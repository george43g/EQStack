...
export const MEETING_HARNESS = [
  ...
  // Step 9 (2026-09-24): the announcement and the tool call must share a turn,
  // or the model sometimes ends its turn on the announcement; and chair lines
  // came out inside a member's tag. pre_tool_speech is "off" for ask_agent.
...
export function buildMeetingToolSpec(cfg: Config): ConsultToolSpec {
  ...
  return {
    ...
    // Step 9: "force" put EL's filler inside the member's voice tag and was
    // then followed by no tool call in 6 of 9 turns. The chair announces.
    preToolSpeech: "off",
  };
}
