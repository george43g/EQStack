...
| O-7 | `pending-window-value` · eqstack | 2s default `delivery_timeout_ms` — right value? (err=22 lands near-synchronously per the diagnosis; delivery receipts can be slower.) Implementer may tune with data | implementer | RS-A polish |
